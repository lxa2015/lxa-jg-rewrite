...
# ----------------------------------------------------------------------------------------------------------------------------#
def SignatureDifference(sig1, sig2,
                        outfile):  # this finds the best alignments between affixes of a signature, and also gives a measure of the similarity.
...
# ----------------------------------------------------------------------------------------------------------------------------#
def printSignatures(Lexicon, lxalogfile, outfile_signatures, outfile_wordstosigs, outfile_stemtowords, encoding,
                    FindSuffixesFlag):
    # ----------------------------------------------------------------------------------------------------------------------------#
    ...
    print >> outfile_signatures, "Print signatures function.\n"
    print >> outfile_signatures, "{:40s}{:10,d}".format("Number of signatures: ", len(DisplayList))
    print >> outfile_signatures, "{:40s}{:10,d}".format("Number of singleton signatures: ", singleton_signatures)
    print >> outfile_signatures, "{:40s}{:10,d}".format("Number of doubleton signatures: ", doubleton_signatures)
    print >> outfile_signatures, "{:40s}{:10,d}".format("Total number of letters in stems: ", Lexicon.LettersInStems)
    print >> outfile_signatures, "{:40s}{:10,d}".format("Total number of letters in signatures: ",
                                                        Lexicon.AffixLettersInSignatures)
    print >> outfile_signatures, "{:40s}{:10,d}".format("Total: ",
                                                        Lexicon.LettersInStems + Lexicon.AffixLettersInSignatures)
    print >> outfile_signatures, "{:40s}{:10,d}".format("Total robustness in signatures ", totalrobustness)

    print >> lxalogfile, "{:40s}{:10,d}".format("Number of signatures: ", len(DisplayList))
    print >> lxalogfile, "{:40s}{:10,d}".format("Number of singleton signatures: ", singleton_signatures)
    print >> lxalogfile, "{:40s}{:10,d}".format("Number of doubleton signatures: ", doubleton_signatures)
    print >> lxalogfile, "{:40s}{:10,d}".format("Total number of letters in stems: ", Lexicon.LettersInStems)
    print >> lxalogfile, "{:40s}{:10,d}".format("Total number of letters in signatures: ",
                                                Lexicon.AffixLettersInSignatures)
    print >> lxalogfile, "{:40s}{:10,d}".format("Total: ", Lexicon.LettersInStems + Lexicon.AffixLettersInSignatures)
    print >> lxalogfile, "{:40s}{:10,d}".format("Total robustness in signatures ", totalrobustness)

    print
    "{:40s}{:10,d}".format("Number of signatures: ", len(DisplayList))
    print
    "{:40s}{:10,d}".format("Number of singleton signatures: ", singleton_signatures)
    print
    "{:40s}{:10d}".format("Number of doubleton signatures: ", doubleton_signatures)
    print
    "{:40s}{:10,d}".format("Total number of letters in stems: ", Lexicon.LettersInStems)
    print
    "{:40s}{:10,d}".format("Total number of letters in signatures: ", Lexicon.AffixLettersInSignatures)
    print
    "{:40s}{:10,d}".format("Total: ", Lexicon.LettersInStems + Lexicon.AffixLettersInSignatures)
    print
    "{:40s}{:10,d}".format("Total robustness in signatures ", totalrobustness)

    print

    print >> outfile_signatures, "\n" + "-" * 150
    print >> outfile_signatures, '{0:<50}{1:>20s} {2:>10s} {3:>25s} {4:>20s} '.format("Signature", "Stem count",
                                                                                      "Robustness",
    ...
    for sig, stemcount, robustness in DisplayList:
        ...
        if encoding == "utf8":
            ...
            print >> outfile_signatures, '{0:<50}{1:10d} {2:15d} {3:25.3%} {4:20.3%}'.format(sig, stemcount, robustness,
                                                                                             robustness / totalrobustness,
                                                                                             runningsum / totalrobustness)
    print >> outfile_signatures, "--------------------------------------------------------------"
    ...
    if (False):
        ...
        print >> outfile_signatures, '{0:<35}{1:12s} {2:12s}'.format("Signature", "Stem count", "Robustness")
        print >> outfile_signatures, "--------------------------------------------------------------"
        for sig, stemcount, robustness in DisplayList:
            ...
            if encoding == "utf8":
                ...
                print >> outfile_signatures, '{0:<35}{1:6d} {2:6d}'.format(sig, stemcount, robustness)
        print >> outfile_signatures, "--------------------------------------------------------------"
    ...
    for sig, stemcount, robustness in DisplayList:
        ...
        (a, b) = findWordListInformationContent(Lexicon.WordList.mylist, bitsPerLetter)
        (c, d) = findSignatureInformationContent(Lexicon.SignatureToStems, sig, bitsPerLetter)
        ...
        print >> outfile_signatures, formatstring % ("Information in words if unanalyzed:", a, b, a + b)
        print >> outfile_signatures, formatstring % ("Information in words as analyzed:", c, d, c + d)
        # -------------------  end of New -----------------------------------------------------------------------------------



        print >> outfile_signatures, "\n-------------------------"
        print >> outfile_signatures, "Entropy-based stability: ", StableSignature(stemlist)
    ...
    if True:
        ...
        for i in range(maxnumberofsigs):
            if i in ambiguity_counts:
                ...
                print
                "{:4d}{:10,d}".format(i, ambiguity_counts[i])
        maxlengths = dict()
        for word in Lexicon.WordToSig:
            listofsignatures = list(Lexicon.WordToSig[word])
            listofsignatures = sorted(listofsignatures, key=len)
            for n in range(len(Lexicon.WordToSig[word])):
                sig = "-".join(list(Lexicon.WordToSig[word][n]))
                if n not in maxlengths:
                    maxlengths[n] = 0
                if len(sig) > maxlengths[n]:
                    maxlengths[n] = len(sig)
        ...
        for word in wordlist:
            # if encoding == "utf8":
            #	print >>outfile_wordstosigs, word,
            #	for sig in WordToSig[word]:
            #		print >>outfile_wordstosigs, sig,
            #	print >>outfile_wordstosigs
            # else:
            print >> outfile_wordstosigs, '{0:<30}'.format(word), ":",
            for n in range(len(Lexicon.WordToSig[word])):
                sig = "-".join(list(Lexicon.WordToSig[word][n]))
                print >> outfile_wordstosigs, sig + " " * (maxlengths[n] - len(sig)),
            print >> outfile_wordstosigs
    ...
    print >> outfile_stemtowords, "--------------------------------------------------------------"
    for stem in stems:
        print >> outfile_stemtowords, '{:10}'.format(stem),
        worlist = Lexicon.StemToWord[stem].keys()
        wordlist.sort()
        for word in wordlist:
            print >> outfile_stemtowords, '{:15}'.format(word),
        print >> outfile_stemtowords
...
# ----------------------------------------------------------------------------------------------------------------------------#
def MakeSignatures(Lexicon, lxalogfile, FindSuffixesFlag, MinimumStemLength):
    # ----------------------------------------------------------------------------------------------------------------------------#
    ...
    if FindSuffixesFlag:
        for i in range(len(Lexicon.WordList.mylist)):
            ...
            for i in range(span):
                if word[i] != previousword[i]:
                    ...
                    stem = word[:i]
                    if len(stem) >= Lexicon.MinimumStemLength:
            ...
            if len(previousword) > i + 1:
                ...
                continue
            if (len(word)) >= i:
                if len(previousword) >= Lexicon.MinimumStemLength:
        ...
        # Sort them from the end!
        i = 0
        for word in Lexicon.WordList:
            differencefoundflag = False
            if previousword == "":  # only on first iteration
                previousword = word
                continue
            span = min(len(word), len(previousword))
            # print word, previousword, span
    ...
    for i in range(len(Lexicon.WordList.mylist)):
        ...
        for i in range(len(word) - 1, Lexicon.MinimumStemLength, -1):
            stem = word[:i]
            if stem in Protostems:
                ...
                if stem not in Lexicon.StemToWord:
                    ...
                    if stem in Lexicon.WordList.mylist:
                        Lexicon.StemToWord[stem][stem] = 1
                ...
                WordAnalyzedFlag = True
        if WordAnalyzedFlag == True:
            Lexicon.NumberOfAnalyzedWords += 1
            Lexicon.LettersInAnalyzedWords += len(word)
        else:
            Lexicon.NumberOfUnanalyzedWords += 1
            Lexicon.LettersInUnanalyzedWords += len(word)
    ...
    for sig in Lexicon.SignatureToStems:
        numberofaffixes = len(sig)
        mystems = Lexicon.SignatureToStems[sig]
        ...
        robustness = StemListLetterLength * (numberofaffixes - 1) + AffixListLetterLength * (numberofstems - 1)
        Lexicon.TotalRobustnessInSignatures += robustness
        Lexicon.AffixLettersInSignatures += AffixListLetterLength

    print >> lxalogfile, "{:30s}{:5d}".format("Number of analyzed words", Lexicon.NumberOfAnalyzedWords)
...
# ----------------------------------------------------------------------------------------------------------------------------#
def MakeSignaturesOld(StemToWord, StemToSig, FindSuffixesFlag, outfile,
                      NoLengthLimitFlag=False):  # This is no longer used. Signatures is a map from the signature to a set of its stems.
    # ----------------------------------------------------------------------------------------------------------------------------#
    # Signatures is a map: its keys are signatures. Its values are *sets* of stems.
    # StemToWord is a map; its keys are stems.      Its values are *sets* of words.
    # WordToSig is a Map. its keys are words.       Its values are *lists* of signatures
    ...
    for stem in StemToWord.keys():
        ...
        if len(thissig) == 0:
            print
            "no length signature line 1381", stem
