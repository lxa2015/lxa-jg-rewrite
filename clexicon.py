...
class CWordList:
    ...
    def sort(self):
        print
        ">>>>>>>>>>>>>>>>>>>>>>>>>..I am sorting"
        self.mylist.sort(key=byWordKey)
        for i in range((self.mylist)):
            word = self.mylist[i]
            word.leftindex = i
        templist = list()
        for word in self.mylist:
            thispair = tuple(word.sort[::-1], word.leftindex)
            templist.append(thispair)
        templist.sort(key=item[0])
...
class CLexicon:
    def __init__(self):
        print
        "initializing clexicon"
        self.WordList = CWordList()
        print
        "initialized self WordList"
