...
# ----------------------------------------------------------------------------------------------------------------------------#
def AverageCountOfTopStems(howmany, sig, Signatures):
...
# ----------------------------------------------------------------------------------------------------------------------------#
def printSignatures(Signatures, WordToSig, SignatureToStem, outfile, outfileGephi):
    # ----------------------------------------------------------------------------------------------------------------------------#
    ...
    for sig, stemcount, robustness in DisplayList:
        ...
        print >> outfile, "\n-------------------------"
        print >> outfile, "Average count of top 5 stems:", AverageCountOfTopStems(5, sig, Signatures)
        ...
        if signo in signumbers:
            for stem in Signatures[sig]:
                colornumber = 0
                for word in StemToWord[stem]:
                    print >> outfileGephi, word, colornumber
                    colornumber += 1
...
def find_N_highest_weight_suffix(wordlist):
    ...
    maximalchunksize = 5  # should be 3 or 4 ***********************************
    totalweight = 0
    threshold = 50
...
language = "english"
infolder = '../../Dropbox/data/' + language + '/'

size = 20  # french 153 10 english 14 46

specificname = "encarta"
infilename = infolder + specificname + "_" + str(size) + "Kwords" + ".txt"
...
outfileGephiname = outfolder + language + "_" + str(size) + "Kwords" + "_wordcolors_gexf.txt"
outfileGephi = open(outfileGephiname, "w")
...
printSignatures(Signatures, WordToSig, Signatures, outfile, outfileGephi)
...
for sig1, numberofstems, robustness in DisplayList:
    ...
    for sig2, numberofstems2, robustness2 in DisplayList:
        ...
        for n in range(len(ShortList)):
            # print ShortList[n]
            if compressedSize < ShortList[n][1]:
                ShortList.insert(n, (sig2, compressedSize))
                # print sig2
                break;
        if n < MaxSizeForShortList:
            ShortList.append((sig2, compressedSize))
